...
**참고**: 최초 실행 시 웹 브라우저가 열리며 초기 설정 화면이 나타납니다. 토큰 입력 후 서버와 채널을 선택하면 멤버 목록이 자동으로 로드됩니다. 이제 유저 ID를 직접 입력할 필요 없이, **목록에서 본인이나 친구를 체크박스로 선택**하기만 하면 설정을 완료할 수 있습니다. 저장이 완료되면 자동으로 봇이 실행됩니다.
